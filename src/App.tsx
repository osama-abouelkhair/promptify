    ...
    <div className="flex h-dvh min-h-0 w-full overflow-hidden bg-slate-50 text-slate-900 font-sans">
      ...
      <aside className={`${isSidebarOpen ? 'flex' : 'hidden'} sidebar-scrollbar md:flex h-full min-h-0 flex-col w-full md:w-64 overflow-y-scroll overscroll-contain bg-slate-700 text-slate-100 fixed md:static inset-0 z-50 md:z-auto transition-all`}>
        <div className="sticky top-0 z-10 shrink-0 bg-slate-700 shadow-sm">
          ...
          <div className="p-4 pb-3 space-y-2">
            ...
            <Link
              ...
              onClick={() => setIsSidebarOpen(false)}
              className={`w-full flex items-center gap-3 px-4 py-2 rounded-md font-medium transition-colors text-left ${isSearchView ? 'bg-slate-600 text-white' : 'text-slate-300 hover:text-white hover:bg-slate-600'}`}
            >
        ...
        <nav className="shrink-0">
          <div className="px-8 pt-4 pb-2 text-xs font-semibold text-slate-300 uppercase tracking-wider">Prompts</div>
          <div className="px-4 pb-4 space-y-2">
            ...
            return (
              <Link
                ...
                onClick={() => setIsSidebarOpen(false)}
                className={`w-full flex items-center gap-3 px-4 py-2 rounded-md font-medium transition-colors text-left ${isPromptActive ? 'bg-slate-600 text-white' : 'text-slate-300 hover:text-white hover:bg-slate-600'} `}
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
